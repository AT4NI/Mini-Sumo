...
byte LineState() {
  ...
  int line1 = analogRead(LINE_L);
  int line2 = analogRead(LINE_R);

  if (line1 < thresholdL && line2 < thresholdR) total = 0;  
  else if (line1 >= thresholdL && line2 < thresholdR) total = 1;
  else if (line1 < thresholdL && line2 >= thresholdR) total = 2; 
  else if (line1 >= thresholdL && line2 >= thresholdR) total = 3;
  return total;
}
